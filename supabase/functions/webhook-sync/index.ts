...
serve(async (req) => {
  ...
  try {
    ...
    const sourceSystem = order.source_systems as { id: string; webhook_url: string | null; webhook_secret: string | null } | null;
    ...
    return new Response(JSON.stringify({ success, response_status: responseStatus }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
